...
public abstract class RSASignature extends SignatureSpi {
    ...
    protected boolean engineVerify(byte[] sigBytes) throws SignatureException {
        ...
        } catch (javax.crypto.BadPaddingException | IOException e) {
            return false;
        } finally {
            resetDigest();
        }
    ...
    static byte[] encodeSignatureWithoutNULL(ObjectIdentifier oid, byte[] digest)
            throws IOException {
        DerOutputStream out = new DerOutputStream();
        DerOutputStream oidStream = new DerOutputStream();
        oidStream.putOID(oid);
        out.write(DerValue.tag_Sequence, oidStream);
        out.putOctetString(digest);
        DerValue result =
                new DerValue(DerValue.tag_Sequence, out.toByteArray());
        return result.toByteArray();
