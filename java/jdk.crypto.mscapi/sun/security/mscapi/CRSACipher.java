...
public final class CRSACipher extends CipherSpi {
    ...
    private byte[] encryptDecrypt(byte[] data, int dataSize,
            long key, boolean doEncrypt) throws KeyException, BadPaddingException {
        int[] returnStatus = new int[1];
        byte[] result;
        result = encryptDecrypt(returnStatus, data, dataSize, key, doEncrypt);
        if (returnStatus[0] == ERROR_INVALID_PARAMETER) {
            if (forTlsPremasterSecret) {
                result = null;
            } else {
